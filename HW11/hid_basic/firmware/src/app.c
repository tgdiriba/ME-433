...
void APP_Initialize ( void )
{
    ...
    appData.FIR[0] = 0.032042;
    appData.FIR[1] = 0.239120;
    appData.FIR[2] = 0.457675;
    appData.FIR[3] = 0.239120;
    appData.FIR[4] = 0.032042;
}
            ...
            else if( appData.hidDataReceived )
            {
                ...
                switch(appData.receiveDataBuffer[0])
                {
                    ...
                    case 0x02:
                        ...
                        appData.FIFO[appData.FIFO_Head] = appData.accelerations[2];
                        appData.FIFO_Head = (appData.FIFO_Head+1)%MAF_SIZE;

                        /*MAF
                        int u, z_sum = 0;
                        for(u = 0; u < FIR_SIZE; u++) {
                            z_sum += appData.FIFO[u];
                        }
                        appData.accelerations[2] = z_sum / MAF_SIZE;
                         */
                        ...
                        float z_sum = 0;
                        for(u = 0; u < FIR_SIZE; u++) {
                            z_sum += appData.FIR[u]*appData.FIFO[u];
                        }
                        appData.accelerations[2] = z_sum;
