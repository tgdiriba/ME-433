...
void display_draw_vbar(int row, int col, int direction, int length, int width) {
    int r, c;
    length = abs(length);
    width = abs(width);
    for(r = row; r != row+signum(direction)*length; r += signum(direction)) {
        for(c = col-width/2; c < col+width/2; c++) {
            if(display_pixel_inrange(r, c)) {
                display_pixel_set(r, c, 1);
            }
            else {
                if(c < 0) c = 0;
                else if(c >= WIDTH) break;
            }
        }
        if(r < 0) r = 0;
        else if(r >= HEIGHT) break;
    }
}

void display_draw_hbar(int row, int col, int direction, int length, int width) {
    int r, c;
    length = abs(length);
    width = abs(width);
    for(r = row-width/2; r < row+width/2; r++) {
        for(c = col; c != col+signum(direction)*length; c += signum(direction)) {
            if(display_pixel_inrange(r, c)) {
                display_pixel_set(r, c, 1);
            }
            else {
